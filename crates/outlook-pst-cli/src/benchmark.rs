use outlook_pst::{
    ltp::prop_context::PropertyValue, messaging::folder::Folder, ndb::node_id::NodeId, open_store,
};
use std::rc::Rc;
...
pub fn run_bench(args: crate::args::BenchArgs) -> anyhow::Result<()> {
    ...
    for pst_path in files {
        ...
        let ipm_sub = match store.properties().ipm_sub_tree_entry_id() {
            Ok(v) => v,
            Err(e) => {
                eprintln!("Skipping store '{}': {}", pst_path.display(), e);
                continue;
            }
        };
        let root = match store.open_folder(&ipm_sub) {
            Ok(f) => f,
            Err(e) => {
                eprintln!("Skipping store '{}': {}", pst_path.display(), e);
                continue;
        ...
        while let Some((folder, path)) = stack.pop() {
            ...
            if let Some(ct) = folder.contents_table() {
                for row in ct.rows_matrix() {
                    ...
                    let t0 = Instant::now();
                    let header_text = props.get(0x007D).and_then(|v| match v {
                        PropertyValue::String8(s) => Some(s.to_string()),
                        PropertyValue::Unicode(u) => Some(u.to_string()),
                        _ => None,
                    });
                    let headers =
                        header_text.map(|t| crate::iterate_emails::parse_transport_headers(&t));
                    let (mut to, mut cc, mut bcc) = (Vec::new(), Vec::new(), Vec::new());
                    if let Some(h) = &headers {
                        to =
                            h.to.as_ref()
                                .map(|s| crate::iterate_emails::parse_recipients_from_header(s))
                                .unwrap_or_default();
                        cc =
                            h.cc.as_ref()
                                .map(|s| crate::iterate_emails::parse_recipients_from_header(s))
                            ...
                            for (col_def, value) in info.columns().iter().zip(cols) {
                                if matches!(col_def.prop_id(), 0x0C15 | 0x3001 | 0x39FE | 0x3003) {
                                    if let Some(valref) = value.as_ref() {
                                        let _ = rt.read_column(valref, col_def.prop_type());
                                    }
                                }
                            }
                        }
    ...
    if total_messages > 0 {
        println!(
            "Header parse avg: {:.2} µs",
            timing_header_us as f64 / total_messages as f64
        );
        if table_forced > 0 {
            println!(
                "Table read avg (only when done): {:.2} µs",
                timing_table_us as f64 / table_forced as f64
            );
        }
        println!(
            "Body extract avg: {:.2} µs",
            timing_body_us as f64 / total_messages as f64
        );
    }
