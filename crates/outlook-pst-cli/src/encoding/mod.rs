    ...
    while i < bytes.len() {
        if i + 4 < bytes.len() && bytes[i] == b'=' && bytes[i + 1] == b'?' {
            ...
            if let Some(rel_q1) = s[start..].find('?') {
                ...
                if let Some(rel_q2) = s[enc_start..].find('?') {
                    // end of encoding token
                    let q2 = enc_start + rel_q2; // position of second '?'
                    if q2 + 2 <= s.len() {
                        // ensure room for data and terminator later
                        let data_start = q2 + 1;
                            ...
                            if !enc_token.is_empty() {
                                let decoded = if enc_token.eq_ignore_ascii_case("B") {
                                    ...
                                    while j < qb.len() {
                                        if qb[j] == b'='
                                            && j + 2 < qb.len()
                                            && qb[j + 1].is_ascii_hexdigit()
                                            && qb[j + 2].is_ascii_hexdigit()
                                        {
                                            if let Ok(v) = u8::from_str_radix(&qp[j + 1..j + 3], 16)
                                            {
                                                buf.push(v);
                                                j += 3;
                                                continue;
                                            }
                                        } else if qb[j] == b'=' && j + 1 == qb.len() {
                                            break;
                                        }
                                        buf.push(qb[j]);
                                        j += 1;
...
fn decode_bytes_charset(data: &[u8], charset: &str) -> Option<String> {
    ...
    if lower.contains("bad_stoic") {
        eprintln!(
            "Error: Found Bad_Stoic charset in encoded-word!\n Charset: '{}'\n Data: {:?}\n",
            charset, data
        );
    }
    match lower.as_str() {
...
pub fn decode_subject(value: &PropertyValue) -> Option<String> {
    match value {
        PropertyValue::String8(raw) => {
            let buf = raw.buffer();
            let slice = if matches!(buf.first(), Some(1)) {
                &buf[1..]
            } else {
                buf
            };
