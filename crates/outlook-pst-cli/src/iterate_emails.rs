...
/// Extension trait for PropertyValue to simplify string extraction
trait PropertyValueExt {
    /// Extract string content from String8 or Unicode variants
...
/// Convert Windows FILETIME (100ns intervals since 1601-01-01) to a formatted datetime string
...
/// Parse raw MIME transport headers text into structured TransportHeaders
...
/// Process a single transport header and store it in the appropriate TransportHeaders field
...
/// Format a mailbox as "Display Name <email@example.com>" or just the email/name if only one is available
...
/// Clean Exchange Distinguished Names (DN) to extract readable display names
...
/// Extract email address from a string like "Name <email@example.com>" or bare email
fn extract_email(s: &str) -> Option<String> {
    let s = s.trim();
    if let Some(start) = s.find('<') {
        if let Some(end) = s[start + 1..].find('>') {
            let email = &s[start + 1..start + 1 + end];
            if email.contains('@') {
                return Some(email.to_ascii_lowercase());
            }
        }
    }
    let cand = s.trim_matches(['"', ' ']);
    if cand.contains('@') {
...
/// Extract sender information from a message, returning (display_name, email_address)
...
/// Extract recipients from a message, returning (To, Cc, Bcc) lists of formatted addresses
fn get_recipients(message: &UnicodeMessage) -> (Vec<String>, Vec<String>, Vec<String>) {
    ...
    for row in recipient_table.rows_matrix() {
        let columns: Vec<_> = match row.columns(context) {
            Ok(cols) => context.columns().iter().zip(cols).collect(),
            Err(_) => {
                continue;
            }
        };
        let recipient_type = columns
            .iter()
            .find_map(|(col, value)| {
            ...
            .and_then(|value| match value {
                PropertyValue::Integer32(val) => Some(val),
                _ => None,
            });
        ...
        let smtp_address = columns
            .iter()
            .find_map(|(col, value)| {
                if col.prop_id() == 0x39FE {
                    Some((value.as_ref(), col.prop_type()))
                } else {
                    None
                }
            ...
            .and_then(|value| value.as_string());
        let pr_email_address = columns
            .iter()
            .find_map(|(col, value)| {
                if col.prop_id() == 0x3003 {
                    Some((value.as_ref(), col.prop_type()))
                } else {
                    None
                }
        ...
        if let Some(recipient_type) = recipient_type {
            ...
            match recipient_type {
                MAPI_TO => to_recipients.push(formatted_recipient),
                MAPI_CC => cc_recipients.push(formatted_recipient),
                MAPI_BCC => bcc_recipients.push(formatted_recipient),
                _ => {}
            }
        }
    }
...
/// Extract and parse transport headers from a message if available
...
/// Parse a comma-separated recipient header value into individual addresses
...
fn list_attachments(message: &Rc<UnicodeMessage>) -> Option<Vec<AttachmentInfo>> {
    ...
    for row in table.rows_matrix() {
        ...
        if let Ok(att) = UnicodeAttachment::read(message.clone(), sub_node, None) {
            ...
            let read_str =
                |id: u16| -> Option<String> { props.get(id).and_then(|v| v.as_string()) };
            let name = read_str(0x3707)
...
/// Detect which body types are available in a message (text, html, rtf)
...
fn extract_plain_body(message: &UnicodeMessage) -> Option<String> {
    ...
    if let Some(val) = props.get(0x1013) {
        let html_opt = if let PropertyValue::Binary(b) = val {
            let code_page = match props.get(0x3FDE) {
                Some(PropertyValue::Integer32(cpid)) => u16::try_from(*cpid).ok(),
                _ => None,
            };
            encoding::decode_html_body(b.buffer(), code_page.unwrap_or(20127))
        } else {
            val.as_string()
        };
...
/// Process a single message: extract headers, generate output for list/dump, update stats
fn process_message(
    ...
    let properties = message.properties();
    // Core headers for HTML output (Subject, From, Date, To, Cc, Bcc, Attachments)
    let mut core_headers: Vec<String> = Vec::new();
    // Extra headers for list mode and optional headers file
    let mut extra_headers: Vec<String> = Vec::new();
    let transport_headers = get_transport_headers(&message);
    let subject = transport_headers
        .as_ref()
        .and_then(|h| h.subject.clone())
        .or_else(|| properties.get(0x0037).and_then(encoding::decode_subject))
        .unwrap_or_else(|| "(No Subject)".to_string());
    let received_time = transport_headers
        .as_ref()
        ...
        .or_else(|| {
            properties
                .get(0x0E06)
                .and_then(|value| match value {
                    PropertyValue::Time(time) => Some(*time),
                    _ => None,
                })
                .map(filetime_to_datetime)
        })
        .unwrap_or_else(|| "Unknown Date".to_string());
    let message_id = transport_headers
        .as_ref()
    ...
    let from_display = if let Some(headers) = &transport_headers {
        if let Some(from_header) = &headers.from {
            from_header.clone()
        } else {
    ...
    // Core headers for HTML: Subject, From, Date
    core_headers.push(format!("Subject: {}", subject));
    core_headers.push(format!("From: {}", from_display));
    core_headers.push(format!("Date: {}", received_time));
    ...
    if !to_recipients.is_empty() {
        core_headers.push(format!("To: {}", to_recipients.join("; ")));
    }
    if !cc_recipients.is_empty() {
        core_headers.push(format!("Cc: {}", cc_recipients.join("; ")));
    }
    if !bcc_recipients.is_empty() {
        core_headers.push(format!("Bcc: {}", bcc_recipients.join("; ")));
    }

    // Collect all participant emails for responsive email matching
    let collect_participant_emails = || -> Vec<String> {
        let mut emails = Vec::new();
        if let Some(e) = extract_email(&from_display) {
            emails.push(e);
        }
        for r in &to_recipients {
            if let Some(e) = extract_email(r) {
                emails.push(e);
            }
        }
        ...
        if let Some(h) = &transport_headers {
            if let Some(ref v) = h.from {
                if let Some(e) = extract_email(v) {
                    emails.push(e);
                }
            }
            if let Some(ref v) = h.to {
    ...
    // Responsive Emails check
    let provided_emails: Vec<String> = match action {
        Action::List(a) => a
            .responsive_emails
            .iter()
    ...
    let responsive_matches: Vec<String> = if provided_emails.is_empty() {
        Vec::new()
    } else {
        let participant_emails = collect_participant_emails();
        provided_emails
            .iter()
            .filter(|target| participant_emails.contains(target))
            .cloned()
            .collect()
    };
    if !responsive_matches.is_empty() {
        extra_headers.push(format!(
            "Responsive Emails: {}",
            responsive_matches.join(", ")
        ));
    }
    ...
    if !provided_keywords.is_empty() {
        if let Some(body) = extract_plain_body(&message) {
            ...
            if !hits.is_empty() {
                extra_headers.push(format!("Keywords: {}", hits.join(", ")));
            }
        }
    }
    if let Some(ref msg_id) = message_id {
        extra_headers.push(format!("MessageId: {}", msg_id));
    }
    extra_headers.push(format!("Folder: {}", folder_path));

    let message_size = properties
        .get(0x0E08)
        .and_then(|value| match value {
            PropertyValue::Integer32(size) => Some(*size as u32),
            _ => None,
        })
        .unwrap_or(0);
    if message_size > 0 {
        extra_headers.push(format!("Size: {} bytes", message_size));
    }
    ...
    if !flag_descriptions.is_empty() {
        extra_headers.push(format!("Flags: {}", flag_descriptions.join(", ")));
    }
    ...
    if let Action::List(list_args) = action {
        if list_args.show_headers {
            if let Some(headers) = &transport_headers {
                if headers.message_id.is_some()
                    || headers.x_mailer.is_some()
                    || headers.x_originating_ip.is_some()
                    || !headers.received_chain.is_empty()
                {
                    extra_list_lines.push("Transport Headers:".to_string());
                    if let Some(in_reply_to) = &headers.in_reply_to {
                        extra_list_lines.push(format!("  In-Reply-To: {}", in_reply_to));
                    }
                    if let Some(references) = &headers.references {
                        extra_list_lines.push(format!("  References: {}", references));
                    }
                    if let Some(mailer) = &headers.x_mailer {
                        extra_list_lines.push(format!("  X-Mailer: {}", mailer));
                    }
                    if let Some(ip) = &headers.x_originating_ip {
                        extra_list_lines.push(format!("  X-Originating-IP: {}", ip));
                    }
                    if let Some(return_path) = &headers.return_path {
                        extra_list_lines.push(format!("  Return-Path: {}", return_path));
                    }
                    for (i, received) in headers.received_chain.iter().take(3).enumerate() {
                        extra_list_lines.push(format!("  Received[{}]: {}", i + 1, received));
                    }
                    for (name, value) in headers.other_headers.iter().take(5) {
                        extra_list_lines.push(format!("  {}: {}", name, value));
                    }
                }
            }
        }
                ...
                if !infos.is_empty() {
                    ...
                    for info in infos.iter() {
                        if info.attach_flags == 0 || info.content_id.is_empty() {
                            extra_list_lines.push(format!(
                                "  - {}, (size: {}, cid: {}, af: {})",
                                info.name, info.size, info.content_id, info.attach_flags
                            ));
                        }
                    }
                }
        ...
        if list_args.show_body_types {
            ...
            if !bodies.is_empty() {
                extra_list_lines.push(format!("Bodies: {}", bodies.join(", ")));
            } else {
                extra_list_lines.push("Bodies: none".to_string());
            }
        }
    }
    if let Action::Dump(_) = action {
        if let Some(infos) = attachments.as_ref() {
            if !infos.is_empty() {
                ...
                for info in infos.iter() {
                    if info.attach_flags == 0 || info.content_id.is_empty() {
                        attachments.push(info.name.to_string());
                    }
                }
                if !attachments.is_empty() {
                    core_headers.push(format!("Attachments: {}", attachments.join(" ;;; ")));
                }
            }
        }
    }
    ...
    if let Some(rows) = csv_rows {
        ...
        let responsive_count = responsive_matches.len();
    ...
    if let Action::Dump(dump_args) = action {
        ...
        if let Some(val) = properties.get(0x1013) {
            body_html = if let PropertyValue::Binary(b) = val {
                let code_page = match properties.get(0x3FDE) {
                    Some(PropertyValue::Integer32(cpid)) => u16::try_from(*cpid).ok(),
                    _ => None,
                };
                encoding::decode_html_body(b.buffer(), code_page.unwrap_or(20127))
        ...
        if body_html.is_none() {
            if let Some(val) = properties.get(0x1000) {
                if let Some(s) = val.as_string() {
                    let t = htmlescape::encode_minimal(&s);
                    body_html = Some(format!("<pre>{}</pre>", t));
                }
            }
        }
        if body_html.is_none() {
            if let Some(PropertyValue::Binary(b)) = properties.get(0x1009) {
                if let Some(rtf) = encoding::decode_rtf_compressed(b.buffer()) {
                    let t = htmlescape::encode_minimal(&rtf);
                    body_html = Some(format!("<pre>{}</pre>", t));
                }
            }
        }
        ...
        // Optional page break between headers and body
        ...
        // Optionally write metadata to a separate text file
        if dump_args.metadata {
            if let Some(parent) = path.parent() {
                let metadata_path = parent.join("metadata.txt");
                let mut metadata_content = String::new();
                // Write core headers (Subject, From, Date, To, Cc, Bcc, Attachments)
                for line in &core_headers {
                    metadata_content.push_str(line);
                    metadata_content.push('\n');
                }
                // Write extra headers (Responsive Emails, Keywords, MessageId, Folder, Size, Flags)
                for line in &extra_headers {
                    metadata_content.push_str(line);
                    metadata_content.push('\n');
                }
                if let Err(e) = fs::write(&metadata_path, &metadata_content) {
                    eprintln!(
                        "Warning: Failed to write metadata file for index {}: {}",
                        index_str, e
                    );
                }
            }
        ...
        // Optionally write transport headers to a separate text file
        if dump_args.headers {
            if let Some(ref h) = transport_headers {
                if let Some(parent) = path.parent() {
                    let headers_path = parent.join("headers.txt");
                    let mut headers_content = String::new();
                    if let Some(ref v) = h.message_id {
                        headers_content.push_str(&format!("Message-ID: {}\n", v));
                    }
                    if let Some(ref v) = h.in_reply_to {
                        headers_content.push_str(&format!("In-Reply-To: {}\n", v));
                    }
                    if let Some(ref v) = h.references {
                        headers_content.push_str(&format!("References: {}\n", v));
                    }
                    if let Some(ref v) = h.x_mailer {
                        headers_content.push_str(&format!("X-Mailer: {}\n", v));
                    }
                    if let Some(ref v) = h.x_originating_ip {
                        headers_content.push_str(&format!("X-Originating-IP: {}\n", v));
                    }
                    if let Some(ref v) = h.return_path {
                        headers_content.push_str(&format!("Return-Path: {}\n", v));
                    }
                    for (i, received) in h.received_chain.iter().enumerate() {
                        headers_content.push_str(&format!("Received[{}]: {}\n", i + 1, received));
                    }
                    for (name, value) in &h.other_headers {
                        headers_content.push_str(&format!("{}: {}\n", name, value));
                    }
                    // Only write if there's content
                    if !headers_content.is_empty() {
                        if let Err(e) = fs::write(&headers_path, &headers_content) {
                            eprintln!(
                                "Warning: Failed to write headers file for index {}: {}",
                                index_str, e
                            );
                        }
                    }
...
/// Ensure a file path is unique by appending (1), (2), etc. if the file already exists
...
/// Save all attachments from a message to the specified directory
fn save_attachments_to_dir(message: &Rc<UnicodeMessage>, out_dir: &Path) -> anyhow::Result<()> {
    ...
    for row in table.rows_matrix() {
        ...
        let read_str = |id: u16| -> Option<String> { props.get(id).and_then(|v| v.as_string()) };
        ...
        let content_id_name = read_str(0x3712).unwrap_or_default();
        let attach_flags: i32 = match props.get(0x3714) {
            Some(PropertyValue::Integer32(flags)) => *flags,
            _ => 0,
        };
...
/// Recursively process a folder and all its subfolders, processing each message
fn process_folder_recursive(
    ...
    if let Some(contents_table) = folder.contents_table() {
        ...
        for row in rows {
            ...
            match store.properties().make_entry_id(node_id) {
                Ok(entry_id) => match UnicodeMessage::read(store.clone(), &entry_id, None) {
                    Ok(message) => {
    ...
    if let Some(hierarchy_table) = folder.hierarchy_table() {
        ...
        for (subfolder_name, node_id_u32) in children {
            let node_id = NodeId::from(node_id_u32);
            match store.properties().make_entry_id(node_id) {
                Ok(entry_id) => match UnicodeFolder::read(store.clone(), &entry_id) {
                    Ok(subfolder) => {
...
/// Collect all PST files from an input path (single file or directory)
...
/// Process a single PST file, extracting messages and updating stats
fn process_single_pst(
    ...
    if let Ok(pst) = UnicodePstFile::open(pst_path.to_string_lossy().as_ref()) {
        ...
        return Err(anyhow::anyhow!(
            "ANSI PST support not implemented in this example yet"
        ));
    }
    Ok(())
}

/// Write CSV data to a file with proper escaping
...
/// Main processing loop: collect PST files, process messages, optionally write CSV
...
/// Run the list subcommand to display email details in the console
pub fn run_list(args: args::ListArgs) -> anyhow::Result<()> {
    run_internal_multi(&args.input, Action::List(&args))
}

/// Run the dump subcommand to export emails as HTML files
