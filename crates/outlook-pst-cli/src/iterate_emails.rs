...
impl PropertyValueExt for PropertyValue {
    ...
    fn as_bool(&self) -> Option<bool> {
        match self {
            ...
            PropertyValue::Integer32(v) => Some(*v != 0),
            _ => None,
        }
    }
}
...
/// Converts a Windows FILETIME (100ns since 1601-01-01 UTC) into a readable UTC string.
...
/// Applies a single header line to the accumulated transport headers.
...
/// Formats a display name and email into a single mailbox string.
...
/// Attempts to shorten/normalize Exchange-style distinguished names.
...
/// Extracts best-effort sender display name and SMTP address.
...
/// Reads the recipient table and returns (To, Cc, Bcc) lists.
...
/// Retrieves raw and parsed transport headers from a message, if present.
fn get_transport_headers(message: &dyn Message) -> Option<TransportHeaderData> {
    let properties = message.properties();
    properties
        .get(0x007D)
        .and_then(|value| value.as_string())
        .map(|headers_text| TransportHeaderData {
            raw: headers_text.clone(),
            parsed: parse_transport_headers(&headers_text),
        })
}
...
/// Collects normalized participant email addresses from headers and recipient lists.
fn gather_participant_emails(
    from_display: &str,
    ...
    let mut push_email = |s: &str| {
        ...
        if let Some(start) = s.find('<') {
            if let Some(end) = s[start + 1..].find('>') {
                let candidate = &s[start + 1..start + 1 + end];
                if candidate.contains('@') {
                    emails.push(candidate.to_ascii_lowercase());
                    return;
                }
            }
        }
        let candidate = s.trim_matches(['"']);
        if candidate.contains('@') {
            emails.push(candidate.to_ascii_lowercase());
        }
    };
...
/// Trims, lowercases, and filters empty target strings.
...
fn find_responsive_matches(participants: &[String], provided: &[String]) -> Vec<String> {
    ...
    provided_norm
        .into_iter()
        .filter(|p| participant_set.contains(p))
        .collect()
}
...
/// Detects which body formats are present (text/html/rtf).
...
/// Extracts a best-effort plain text body for keyword scanning.
...
/// Crude HTML tag stripper used for plain-text fallback.
...
/// Processes a single message for list/export modes, emitting outputs and updating stats.
fn process_message(
    ...
    let parsed_headers = transport_headers.as_ref().map(|h| &h.parsed);
    ...
    let received_time = parsed_headers
    ...
    let from_display = if let Some(headers) = parsed_headers {
    ...
    let force_full = match action {
        Action::List(a) => a.full_recipients,
        Action::Export(a) => a.full_recipients,
    };
    let header_recipients = parsed_headers.map(recipient_lists_from_headers);
    let table_recipients = get_recipients(message.as_ref());
    let (to_recipients, cc_recipients, bcc_recipients) = match (header_recipients, force_full) {
        (Some((mut to, mut cc, mut bcc)), true) => {
            merge_unique(&mut to, &table_recipients.0);
            merge_unique(&mut cc, &table_recipients.1);
            merge_unique(&mut bcc, &table_recipients.2);
            (to, cc, bcc)
        }
        (Some((to, cc, bcc)), false) => {
        ...
        (None, _) => table_recipients,
    };

    let responsive_inputs: Vec<String> = match action {
        Action::List(a) => a.responsive_emails.clone(),
        Action::Export(a) => a.responsive_emails.clone(),
    };
    let responsive_matches = find_responsive_matches(
        &gather_participant_emails(
            &from_display,
            &to_recipients,
            &cc_recipients,
            &bcc_recipients,
    ...
    let keyword_inputs: Vec<String> = match action {
    ...
    let attachment_names: Vec<String> = attachments
        ...
        .map(|infos| {
            infos
                .iter()
                .filter(|info| !info.is_inline)
                .map(|info| info.name.clone())
                .collect::<Vec<String>>()
        })
        .unwrap_or_default();
    let attachments_line = if attachment_names.is_empty() {
        None
    } else {
        Some(attachment_names.join(" ;;; "))
    };

    // Collect formatted fields once so we can reuse across console, HTML, metadata, and CSV
    let mut summary_fields: Vec<(String, String)> = Vec::new();
    ...
    let text_out: Vec<String> = summary_fields
        .iter()
        .filter(|(k, _)| !matches!(action, Action::List(_)) || k != "Attachments")
        .map(|(k, v)| format!("{}: {}", k, v))
        .collect();
    ...
    if let Action::List(list_args) = action {
        if list_args.show_headers {
            if let Some(headers) = &parsed_headers {
                    ...
                    for info in infos.iter() {
                        if !info.is_inline {
                            extra_list_lines.push(format!(
                                "  - {} (size: {}, method: {}, cid: {})",
                                info.name, info.size, info.attach_method, info.content_id
                            ));
                        }
                    }
                }
    ...
    match action {
        ...
        Action::Export(export_args) => {
            let _ = &export_args.out_dir; // silence unused warning before export branch
        }
    }
    ...
    if let Action::Export(export_args) = action {
        ...
        for (key, value) in &html_fields {
            ...
            html.push_str(&htmlescape::encode_minimal(value.trim()));
            html.push_str("</td></tr>");
        }
        ...
        if export_args.metadata {
            if let Some(parent) = path.parent() {
                write_metadata_file(parent, &summary_fields)?;
            }
        }

        if export_args.headers {
            if let (Some(parent), Some(headers)) = (path.parent(), &transport_headers) {
                if !headers.raw.trim().is_empty() {
                    write_headers_file(parent, &headers.raw)?;
                }
            }
        }
    }
...
/// Writes selected non-transport metadata to `metadata.txt` in the target directory.
fn write_metadata_file(dir: &Path, fields: &[(String, String)]) -> anyhow::Result<()> {
    let allowed = [
        "Subject",
        "From",
        "Date",
...
/// Writes raw transport headers to `headers.txt` in the target directory.
fn write_headers_file(dir: &Path, raw_headers: &str) -> anyhow::Result<()> {
    fs::write(dir.join("headers.txt"), raw_headers)?;
    Ok(())
}
...
/// Resolves a single PST or all PST files under a directory, sorted deterministically.
...
/// Opens a PST store and processes its IPM subtree.
...
/// Shared driver for list/export commands over one or more PST files.
