...
- `--conversations` flag for `pst-cli export` to group multi-message threads into deterministic `conv_#####` directories
- Conversation key precedence using bytes 6-21 of `PidTagConversationIndex` with fallback to `PidTagConversationId`
- Conditional `ConversationId:` metadata output when canonical conversation id is present
- Unit and integration coverage for grouping behavior, fallback derivation, singleton layout, and mixed datasets
- Benchmark coverage for conversation-grouping overhead
