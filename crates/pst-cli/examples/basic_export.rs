...
//! Usage:
//! ```bash
//! cargo run --example basic_export -- <pst-file> <output-directory> [--conversations]
//! ```
//!
//! Example:
//! ```bash
//! cargo run --example basic_export -- my_archive.pst ./export_output --conversations
//! ```
...
fn main() {
    ...
    if args.len() < 3 || args.len() > 4 {
        eprintln!(
            "Usage: {} <pst-file> <output-directory> [--conversations]",
            args[0]
        );
        eprintln!();
        eprintln!("Example:");
        eprintln!("  {} archive.pst ./my_export --conversations", args[0]);
        eprintln!();
    ...
    if let Some(ext) = input_path.extension() {
        if ext.to_str().unwrap_or("").to_lowercase() != "pst" {
            eprintln!(
                "Warning: File does not have .pst extension: {}",
                input_path.display()
            );
    ...
    let conversations = args.get(3).is_some_and(|arg| arg == "--conversations");
