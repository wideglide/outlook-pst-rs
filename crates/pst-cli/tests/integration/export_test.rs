...
fn test_basic_single_file_export() {
    ...
    assert!(!entries.is_empty(), "At least one message should be exported");
    ...
    let numbered_dirs: Vec<_> = entries.iter()
        .filter(|e| {
            e.path().is_dir() && 
            e.file_name().to_str().unwrap_or("").chars().all(|c| c.is_ascii_digit())
        })
        .collect();
    
    if !numbered_dirs.is_empty() {
        // Verify at least one numbered directory exists
        assert!(!numbered_dirs.is_empty(), "Should have at least one numbered message directory");
        ...
        if message_html.exists() {
            ...
            let content = fs::read_to_string(&message_html)
                .expect("Should be able to read message.html");
            
            assert!(!content.is_empty(), "message.html should not be empty");
            assert!(content.contains("<html"), "message.html should contain HTML content");
            assert!(content.contains("</html>"), "message.html should be complete HTML");
        }
    }
}
...
fn test_export_message_html_content() {
    ...
    let entries: Vec<_> = fs::read_dir(&output_path)
        ...
        .filter(|e| {
            e.path().is_dir() && 
            e.file_name().to_str().unwrap_or("") != "duplicates"
        })
        .collect();
...
fn test_export_handles_empty_output_dir() {
    ...
    let result = coordinator.run(&mut reporter);
    assert!(result.is_ok(), "Export should create output directory if it doesn't exist");
...
fn test_export_deterministic_numbering() {
    ...
    coordinator1.run(&mut reporter1).expect("First export should succeed");
    ...
    coordinator2.run(&mut reporter2).expect("Second export should succeed");
...
fn test_batch_export_multiple_pst_files() {
    ...
    assert!(!entries.is_empty(), "Should have exported messages from batch");
}
...
fn test_batch_export_sequential_numbering() {
    ...
    if entries.len() >= 2 {
        ...
        let mut numbers: Vec<u32> = entries.iter()
            .filter_map(|e| {
                e.file_name()
                    .to_str()
...
fn test_duplicate_directory_structure() {
    ...
    coordinator.run(&mut reporter).expect("Export should succeed");
...
fn test_no_false_duplicates() {
    ...
    coordinator.run(&mut reporter).expect("First export should succeed");
...
fn test_export_with_metadata() {
    ...
    // Run export
    coordinator.run(&mut reporter).expect("Export with metadata should succeed");
    ...
    assert!(!entries.is_empty(), "Output directory should contain exported messages");
    
    // Check that metadata.txt files exist where expected
    for entry in entries {
        let path = entry.path();
        if path.is_dir() && !path.file_name().unwrap().to_str().unwrap().starts_with("duplicates") {
            ...
            if path.join("message.html").exists() {
                ...
                assert!(content.contains("Subject:"), "Metadata should contain Subject field");
                assert!(content.contains("From:"), "Metadata should contain From field");
            }
        }
    }
}
...
fn test_export_with_headers() {
    ...
    // Run export
    coordinator.run(&mut reporter).expect("Export with headers should succeed");
...
fn test_export_with_all_options() {
    ...
    // Run export
    coordinator.run(&mut reporter).expect("Export with all options should succeed");
    ...
    for entry in entries {
        ...
        if path.is_dir() && !path.file_name().unwrap().to_str().unwrap().starts_with("duplicates") {
            ...
            assert!(
                message_html.exists(),
                "Message directory should contain message.html: {:?}",
                path
            );
            ...
            if attachments_dir.exists() {
                assert!(
                    ...
                    "Attachments should be a directory: {:?}",
                    attachments_dir
                );
            }
...
fn test_export_with_csv() {
    ...
    // Run export
    coordinator.run(&mut reporter).expect("Export with CSV should succeed");
    
    // Verify CSV file exists
    let csv_path = output_path.join("emails.csv");
    assert!(
        csv_path.exists(),
        "CSV file should exist at {:?}",
        csv_path
    );
    ...
    if lines.len() > 1 {
        ...
        for (idx, line) in lines.iter().enumerate().skip(1) {
            ...
            assert!(
                !line.is_empty(),
                "CSV data row {} should not be empty",
                idx
            );
        }
    }
...
fn test_csv_duplicate_flag_accuracy() {
    ...
    assert!(false_count > 0, "Should have at least some non-duplicate messages");
