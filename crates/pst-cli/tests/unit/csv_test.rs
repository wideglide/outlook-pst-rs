...
fn test_csv_row_simple_data() {
    ...
    assert_eq!(
        lines[1],
        "1,Test Subject,sender@example.com,recipient@example.com,2026-02-11T10:00:00Z,msg123@example.com,false,0,0"
    );
}
...
fn test_csv_duplicate_status_column() {
    ...
    assert!(lines[1].contains(",false,"), "First row should be false for duplicate");
    assert!(lines[2].contains(",true,"), "Second row should be true for duplicate");
}
...
fn test_csv_keyword_and_email_counts() {
    ...
    // Should end with ,3,2 for keyword and email counts
    assert!(lines[1].ends_with(",3,2"));
}
