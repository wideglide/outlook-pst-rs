...
use pst_cli::duplicate::{extract_message_id, generate_message_identifier, DuplicateTracker};
...
fn test_duplicate_tracker_multiple_duplicates() {
    ...
    assert_eq!(
        first,
        Some(1),
        "All duplicates should reference first occurrence"
    );
}
...
fn test_duplicate_tracker_unique_count() {
    ...
    assert_eq!(
        ...
        3,
        "Should count only unique messages"
    );
}
...
fn test_fallback_to_hash_for_empty_message_id() {
    let message = MessageData {
        ...
        bcc: vec![],
        date: "2026-02-11".to_string(),
        message_id: Some(String::new()), // Empty string
        body_html: None,
        body_rtf: None,
        body_plain: Some("Body".to_string()),
        attachments: vec![],
